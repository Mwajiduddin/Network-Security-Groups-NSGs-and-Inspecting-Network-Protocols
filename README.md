...
We can access Ubuntu VM from the Windows VM by the Powershell command line using SSH. In Wireshark, filter the traffic by typing in "ssh" in the top bar and click the green fin icon to refresh. Then in Powershell, type  in "ssh (Ubuntu VM username)@(private IP address)" and hit Enter. You will receive a prompt if you want to continue so type "yes." Then you will need to enter the password that you made for your Ubuntu VM under "Administrator account" and when you type in your password you'll notice that there's no visible text appearing, there is nothing wrong with Powershell you just need to type it in correctly. After you've type in your password and hit Enter, you successfully logged in Ubuntu's VM marked by green text virtual machine name. You can list the directories (folders) of Ubuntu VM by typing in "ls -lasth" and press Enter and as you do you will notice the traffic displayed in Wireshark. You can exit out of Ubuntu VM by typing in "exit" and pressing Enter.
